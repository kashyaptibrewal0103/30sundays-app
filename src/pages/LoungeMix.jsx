...
import { MIX_STYLES } from "../components/Community/MixStyles";
import { getDest, questionsFor, tagsFor } from "../data/communityData";
import { useCommunity } from "../state/useCommunity";

// The app's handwriting, not its screens. Same type, same neutrals, same
// signal colours, one bit of personality each.

export default function LoungeMix() {
  const navigate = useNavigate();
  const [params, setParams] = useSearchParams();
  const { answersFor } = useCommunity();

  const asked = params.get("s");
  const v = MIX_STYLES.find(x => x.id === asked) || MIX_STYLES[0];
  ...
  return (
    <Screen>
      <TopBar
        title="Lounge, cohesive"
        sub={`${v.n} of ${MIX_STYLES.length} · ${v.name}`}
