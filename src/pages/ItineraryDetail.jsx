...
import { Camera, Volume2, VolumeX, FileText, ChevronLeft, Compass, Route, BedDouble, Headphones, Map as MapIcon, IndianRupee } from "lucide-react";
...
// The screen's own table of contents. Everything above the day plan is the
// overview, so it has no anchor of its own: tapping it goes back to the top.
const NAV_SECTIONS = [
  { key: "overview", label: "Overview", Icon: Compass },
  { key: "itinerary", label: "Itinerary", Icon: Route },
  { key: "hotels", label: "Hotels", Icon: BedDouble },
  { key: "together", label: "Plan together", Icon: Users },
  { key: "flights", label: "Flights", Icon: Plane },
  { key: "consultant", label: "Consultant", Icon: Headphones },
  { key: "reviews", label: "Reviews", Icon: Star },
  { key: "map", label: "Journey map", Icon: MapIcon },
...
export default function ItineraryDetail({ selectedFlights, selectedHotels, setSelectedHotels }) {
  ...
  // Anchors for the section nav. Held in a ref rather than state: they are read
  // on scroll, never rendered from.
  const sectionEls = useRef({});
  const setSectionEl = useCallback((key) => (el) => { sectionEls.current[key] = el; }, []);
  const elForSection = useCallback((key) => sectionEls.current[key] || null, []);
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      {/* ═══ 2.5 Section nav ═══
          Anchored here and lifted out of the flow, so it takes no space in the
          overview. It shows itself once the day plan comes up to meet it. */}
      <SectionNav sections={NAV_SECTIONS} elFor={elForSection} />
      ...
      <div ref={setSectionEl("itinerary")} style={{ padding: "0 16px" }}>
      ...
      {/* ═══ 4. Hotels ═══ */}
      <div id="hotels-section" ref={setSectionEl("hotels")}>
      ...
      <div ref={setSectionEl("consultant")}>
      ...
      {/* ═══ 7. Traveler Stories ═══ */}
      <div ref={setSectionEl("reviews")} style={{ padding: "0 16px" }}>
      ...
      {/* ═══ 6. Journey Map ═══ */}
      <div ref={setSectionEl("map")} style={{ padding: "0 16px" }}>
      ...
      {/* ═══ 8. Cost breakdown ═══ */}
      <div id="cost-breakdown" ref={setSectionEl("cost")} style={{ padding: "0 16px", scrollMarginTop: 12 }}>
