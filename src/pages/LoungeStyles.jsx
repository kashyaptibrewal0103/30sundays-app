...
import { STYLE_DIRECTIONS } from "../components/Community/StyleDirections";
import { getDest, questionsFor } from "../data/communityData";
import { useCommunity } from "../state/useCommunity";

// Seven personalities for the same screen. One at a time, full width, with the
// real questions in them, because a style can only be judged at the size and
// with the content it has to carry.

export default function LoungeStyles() {
  const navigate = useNavigate();
  const [params, setParams] = useSearchParams();
  const { answersFor } = useCommunity();

  const asked = params.get("s");
  const v = STYLE_DIRECTIONS.find(x => x.id === asked) || STYLE_DIRECTIONS[0];
  ...
  return (
    <Screen>
      <style>{`
        @keyframes loungeBlink { 0%,80%,100% { opacity: 0.25 } 40% { opacity: 1 } }
        @keyframes loungeRise { from { opacity: 0; transform: translateY(9px) } to { opacity: 1; transform: none } }
      `}</style>

      <TopBar
        title="Lounge styles"
        sub={`${v.n} of ${STYLE_DIRECTIONS.length} · ${v.name}`}
        onBack={() => navigate("/community/thailand")}
      />

      <Body>
        {/* The switcher stays put so two options can be compared by tapping
            between them rather than by scrolling and remembering. */}
        <div style={{
        ...
        {/* The screen itself, edge to edge, exactly as wide as it would really be. */}
        <v.Comp d={d} qs={qs} countOf={countOf} />

        <div style={{ padding: `4px ${PAD}px 40px` }}>
